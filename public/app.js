...
let mainContent, authContent, loginForm, registerForm, resetForm,
    ...
    productListStatus, k3Form, formTitle, submitButton,
    bahayaListContainer, rambuListContainer,
    togglePublicArchiveBtn,
    downloadRekapExcelBtn; 
...
function addRow(templateId, containerId, setupListenersCallback) {
    ...
    // Callback ini bisa digunakan untuk mengisi data, tapi juga HARUS memasang listener
...
function setupBahayaListeners(cardElement) {
    ...
    calculateRisk(cardElement); // Hitung awal jika sudah ada data
}
function setupRambuListeners(cardElement) {
    ...
    updateRambuScore(cardElement); // Hitung awal jika sudah ada data
}
    ...
    unsubscribeReviewListener = query.orderBy('createdAt', 'desc').onSnapshot(snapshot => {
        ...
        snapshot.forEach(doc => {
            const data = doc.data();
                ...
                // Cek apakah user perlu melakukan review (Ketua K3 atau Kabag TU melihat laporan yang pending)
                const isReviewNeeded = (userRole === 'KetuaK3' && data.statusLaporan === 'Submitted') || (userRole === 'KabagTU' && data.statusLaporan === 'Reviewed_K3');
                
                const downloadNote = `<span class="download-note" style="font-size: 0.7rem; display: block; text-align: right;">Dokumen sah tanpa ttd fisik stlh disetujui Kabag TU.</span>`;
                ...
                li.innerHTML = `
                    ...
                    <div style="display:flex; flex-direction: column; gap:5px; align-items: flex-end;"> 
                        <div> 
                            ${isRevisable ? `<button class="btn btn-secondary btn-sm btn-revisi" data-id="${doc.id}">Revisi</button>` : ''}
                            ${isReviewNeeded ? `<button class="btn btn-secondary btn-sm btn-lihat-detail" data-id="${doc.id}">Lihat Detail</button>` : ''}
                ...
                // Jika pelaksana, list tidak perlu clickable untuk detail review, hanya info saja (kecuali tombol revisi)
    ...
    try {
        ...
        if (d.daftarBahaya && d.daftarBahaya.length > 0) {
            bahayaHTML = `<table class="detail-table">
                <thead><tr><th>No</th><th>Identifikasi</th><th>Risiko</th><th>Tingkat</th><th>Pengendalian</th></tr></thead>
                <tbody>`;
            d.daftarBahaya.forEach((item, index) => {
                const riskClass = item.tingkatRisiko ? `risk-${item.tingkatRisiko}` : '';
                bahayaHTML += `<tr>
                    <td>${index + 1}</td>
                    <td>${item.identifikasi || '-'}</td>
                    <td>${item.risiko || '-'}</td>
                    <td><span class="risk-output ${riskClass}" style="display:inline-block; width:30px; text-align:center;">${item.tingkatRisiko || '-'}</span></td>
        ...
        if (d.inspeksiRambu && d.inspeksiRambu.length > 0) {
            rambuHTML = `<table class="detail-table">
                <thead><tr><th>No</th><th>Rambu</th><th>Lokasi</th><th>Skor</th><th>Hasil</th></tr></thead>
                <tbody>`;
            d.inspeksiRambu.forEach((item, index) => {
                let resultClass = '';
                if(item.hasil === 'Bagus') resultClass = 'result-bagus';
                else if(item.hasil === 'Masih Layak') resultClass = 'result-layak';
                else resultClass = 'result-perbaikan';

                rambuHTML += `<tr>
                    <td>${index + 1}</td>
                    <td>${item.namaRambu || '-'} (${item.jenisRambu || '-'})</td>
                    <td>${item.lokasi || '-'}</td>
                    <td>${item.totalSkor || '0'}</td>
                    <td><span class="result-output ${resultClass}" style="padding: 2px 5px; font-size: 0.8rem;">${item.hasil || '-'}</span></td>
                </tr>`;
            });
        ...
        reviewReportContent.innerHTML = `
            <div style="background: #f9f9f9; padding: 15px; border: 1px solid #eee; border-radius: 8px;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 10px;">
                    <div><small>Pelaksana:</small><br>${d.pelaksana || '-'}</div>
                    <div><small>Satker:</small><br>${d.satker || '-'}</div>
                ...
                <h4 class="detail-header">2. Inspeksi Rambu</h4>
                ${rambuHTML}

                ${d.catatanK3 ? `<div style="background: #fff3cd; padding: 10px; margin-top: 10px; border-left: 4px solid #ffc107;"><p style="margin:0;"><em>Catatan K3: ${d.catatanK3}</em></p></div>` : ''}
                ${d.catatanTU ? `<div style="background: #d1e7dd; padding: 10px; margin-top: 10px; border-left: 4px solid #198754;"><p style="margin:0;"><em>Catatan TU: ${d.catatanTU}</em></p></div>` : ''}
                
                <p style="margin-top: 1rem;"><a href="/api/laporan/${rId}/download" target="_blank" class="btn btn-secondary btn-sm">Download PDF Asli</a> ${downloadNote}</p>
            </div>`;
...
async function handleApproval(isApproved) {
    ...
    if(userRole==='KetuaK3') {
        ...
        updateData.disetujuiOlehTU_uid=null; updateData.disetujuiOlehTU_nama=null; updateData.tanggalDisetujuiTU=null; updateData.catatanTU=null;
    } else { // KabagTU
        ...
        if(!isApproved) {
             ...
             updateData.diperiksaOlehK3_uid=null; updateData.diperiksaOlehK3_nama=null; updateData.tanggalDiperiksaK3=null; updateData.catatanK3=null;
        }
    }
...
async function handleDownloadRekapPDF() {
    ...
    try {
        ...
        downloadRekapBtn.textContent='Download PDF'; 
...
async function handleDownloadRekapExcel() {
    const user = auth.currentUser;
    if (!user) { alert("Sesi tidak valid. Silakan login ulang."); return; }
    if(!rekapBulanSelect || !rekapTahunSelect || !downloadRekapExcelBtn || !rekapStatus) { console.error("Elemen rekap Excel tidak ditemukan!"); return; }
    
    const bln = rekapBulanSelect.value;
    const thn = rekapTahunSelect.value;
    const fName = `rekap-k3-mij-${thn}-${bln}.xlsx`;

    downloadRekapExcelBtn.textContent = 'Membuat Excel...';
    downloadRekapExcelBtn.disabled = true;
    rekapStatus.textContent = 'Mengambil data...';
    rekapStatus.className = 'loading';

    try {
        const token = await user.getIdToken();
        const response = await fetch(`/api/rekap/excel-bundle?month=${bln}&year=${thn}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });

        if (response.ok) {
            rekapStatus.textContent = 'Memproses file...';
            const blob = await response.blob();
            if (blob.type !== 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' || blob.size === 0) {
                const errorText = await response.text();
                console.error("Server tidak mengembalikan XLSX:", errorText);
                throw new Error(errorText || "Server tidak mengembalikan file Excel yang valid.");
            }
            ...
            a.remove();
            rekapStatus.textContent = `Berhasil mengunduh: ${fName}`;
            rekapStatus.className = 'success';
        } else {
            const errTxt = await response.text();
            console.error(`Gagal download rekap Excel (${response.status}):`, errTxt);
            throw new Error(errTxt || `Gagal mengunduh (${response.status})`);
        }
    } catch (error) {
        console.error("Gagal total download rekap Excel:", error);
        rekapStatus.textContent = `Gagal: ${error.message}`;
        rekapStatus.className = 'error';
    } finally {
        downloadRekapExcelBtn.textContent = 'Download Excel';
        downloadRekapExcelBtn.disabled = false;
    }
}
...
function setupLoggedInListeners() {
    ...
    if (userRole !== 'Pelaksana') {
        ...
        setupListener(downloadRekapExcelBtn, 'click', handleDownloadRekapExcel);
    }
    ...
    setupListener(reviewListUl, 'click', (event) => {
        ...
        // REVISI: Handle klik tombol Revisi DAN tombol Lihat Detail
        const revisiBtn = target.classList.contains('btn-revisi');
        const detailBtn = target.classList.contains('btn-lihat-detail');
        const reportId = revisiBtn || detailBtn ? target.dataset.id : listItem?.dataset.id;
        
        if (revisiBtn && reportId) { loadReportForRevision(event, reportId); }
        // Jika klik tombol detail ATAU klik baris (untuk approver), buka detail view
        else if ((detailBtn || listItem) && userRole !== 'Pelaksana' && reportId) { showReportDetail(reportId); }
    });
...
function initializeAppUI() {
    ...
    downloadRekapExcelBtn = document.getElementById('download-rekap-excel-btn'); 
